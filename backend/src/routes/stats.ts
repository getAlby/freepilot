...
async function calculateStats(prisma: PrismaClient): Promise<CachedStats> {
  ...
  for (const job of completedJobs) {
    try {
      ...
      const tokenMatches = logContent.match(/tokens?["\s:]+(\d+)/gi);
      const satMatches = logContent.match(/sats?["\s:]+(\d+)/gi);
      const costMatches = logContent.match(/cost["\s:]+(\d+)/gi);
      ...
      if (costMatches) {
        for (const match of costMatches) {
          const costCount = parseInt(match.match(/(\d+)/)?.[1] || '0');
          if (!isNaN(costCount)) {
            totalEarningsInSats += costCount;
          }
        }
      }
    } catch (error) {
    ...
    for (const job of completedJobs) {
      if (job.prUrl) {
        try {
          ...
          const prUrlMatch = job.prUrl.match(/github\.com\/([^/]+)\/([^/]+)\/pull\/(\d+)/);
          if (prUrlMatch) {
